...
NEI_1999 <- subset(NEI, year == 1999, select=c(Emissions))
NEI_2002 <- subset(NEI, year == 2002, select=c(Emissions))
NEI_2005 <- subset(NEI, year == 2005, select=c(Emissions))
NEI_2008 <- subset(NEI, year == 2008, select=c(Emissions))

total_emissions <- c(sum(NEI_1999$Emissions),
                     sum(NEI_2002$Emissions),
                     sum(NEI_2005$Emissions),
                     sum(NEI_2008$Emissions))
...
plot(total_emissions_per_year$year, total_emissions_per_year$Emissions,
     type="b",
     lwd=3,
     main="PM2.5 total emissions in the USA",
     xaxt="n",
     yaxt="n",
     xlab="year",
     ylab="")
